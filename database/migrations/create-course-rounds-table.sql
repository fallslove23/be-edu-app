...
CREATE POLICY IF NOT EXISTS "course_rounds_read_all" ON course_rounds
  FOR SELECT USING (true);

CREATE POLICY IF NOT EXISTS "course_sessions_read_all" ON course_sessions
  FOR SELECT USING (true);

-- RLS 정책: 관리자와 운영자만 쓰기 가능
CREATE POLICY IF NOT EXISTS "course_rounds_write_admin" ON course_rounds
  FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM users
...
CREATE POLICY IF NOT EXISTS "course_sessions_write_admin" ON course_sessions
  FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM users
