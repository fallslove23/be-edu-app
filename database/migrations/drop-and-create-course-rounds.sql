-- =============================================
-- Course Rounds (차수) 테이블 재생성
-- 기존 테이블 삭제 후 새로 생성
-- =============================================

-- 기존 테이블 삭제 (CASCADE로 관련 데이터도 함께 삭제)
DROP TABLE IF EXISTS course_sessions CASCADE;
DROP TABLE IF EXISTS course_rounds CASCADE;
...
-- 인덱스 생성
CREATE INDEX idx_course_rounds_template_id ON course_rounds(template_id);
CREATE INDEX idx_course_rounds_status ON course_rounds(status);
CREATE INDEX idx_course_rounds_start_date ON course_rounds(start_date);
CREATE INDEX idx_course_rounds_end_date ON course_rounds(end_date);
CREATE INDEX idx_course_rounds_instructor_id ON course_rounds(instructor_id);
CREATE INDEX idx_course_rounds_manager_id ON course_rounds(manager_id);

CREATE INDEX idx_course_sessions_round_id ON course_sessions(round_id);
CREATE INDEX idx_course_sessions_session_date ON course_sessions(session_date);
